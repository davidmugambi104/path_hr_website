...
export function LiveChat(): JSX.Element {
  ...
  return (
    <>
      ...
      <button
        onClick={openChat}
        className="fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full bg-accent shadow-lg hover:bg-primary transition-all transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent"
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-full max-w-md bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
          {/* Chat Header */}
          ...
          <div className="p-4 border-t border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
            <div className="flex">
              ...
              />
              <button className="bg-accent text-white px-4 py-2 rounded-r-lg hover:bg-primary transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
